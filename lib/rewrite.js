...
function isWildcard(node) {
  return node.type == "Identifier" && /^\$[a-z]+$/.test(node.name) && !_.contains(wildcardsIgnored, node.name); // FORK: https://github.com/rdio/jsfmt/issues/167
}
...
exports.ignoreWildcards = function(){
  wildcardsIgnored = wildcardsIgnored.concat(arguments);
};