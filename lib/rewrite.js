...
function isWildcard(node) {
  return node.type == "Identifier" && /^\$[a-z]+$/.test(node.name) && !_.contains(IGNORED_WILDCARDS, node.name); // FORK: https://github.com/rdio/jsfmt/issues/167
}
...
exports.ignoreWildcards = function(){
  IGNORED_WILDCARDS = IGNORED_WILDCARDS.concat(arguments);
};